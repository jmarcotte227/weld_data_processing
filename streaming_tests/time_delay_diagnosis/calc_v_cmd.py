...
def main():
    ...
    DATASET = "2026_01_16_10_31_35_WLJ_XX_new_functions"
    ...
    for layer in tqdm(range(40,44)):
        cap_data=f"{REC_DIR}{DATASET}/layer_{layer}/"
        cart_vels = []
        job_no = []
        joint_angle = np.loadtxt(cap_data+'weld_js_cmd.csv', delimiter=',')
    ...
    np.savetxt(f"calc_v_cmd/{DATASET}_vel_calc_cmd.csv",vel_set[:,TRIM:-TRIM], delimiter=',')
